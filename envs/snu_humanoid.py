...
class SNUHumanoidEnv(DFlexEnv):
    ...
    def __init__(self, render=False, device='cuda:0', num_envs=4096, seed=0, episode_length=1000, no_grad=True, stochastic_init=False, MM_caching_frequency = 1):
        ...
        if (self.visualize):
            ...
            self.renderer = df.render.UsdRenderer(self.model, self.stage, "snu_humanoid_2")
    ...
    def init_sim(self):
        ...
        self.relative_asset_folder = './assets/snu'
                ...
                for s in self.skeletons:
                    for mesh, link in s.mesh_map.items():
                        ...
                        if link != -1:
                            ...
                            mesh_path = os.path.join(self.relative_asset_folder, "OBJ/" + mesh + ".usd")

                            self.renderer.add_mesh(mesh, mesh_path, X_sc, 1.0, round(self.render_time * self.renderer.fps))
                    ...
                    for m in range(len(s.muscles)):
                        ...
                        self.renderer.add_line_strip(points, name=s.muscles[m].name + str(skel_index), radius=0.0075, color=(self.model.muscle_activation[muscle_start + m]/self.muscle_strengths[m], 0.2, 0.5), time=round(self.render_time * self.renderer.fps))
