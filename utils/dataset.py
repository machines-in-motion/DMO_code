...
class CriticAdvDataset:
    def __init__(self, batch_size, obs, act, target_values, advantages, shuffle = False, drop_last = False):
        self.obs = obs.view(-1, obs.shape[-1])
    ...
    def __getitem__(self, index):
        start_idx = index * self.batch_size
        end_idx = min((index + 1) * self.batch_size, self.obs.shape[0])
        return {'obs': self.obs[start_idx:end_idx, :], 'act': self.act[start_idx:end_idx, :], 'target_values': self.target_values[start_idx:end_idx], 'target_advantages': self.advantages[start_idx:end_idx]}
